...
if [ -z "$1" ]; then
    ...
    echo "用法: bash download_sra.sh <srr_list_file>"
    exit 1
fi
...
awk -F'\t' 'NR==1 {for (i=1; i<=NF; i++) if ($i=="run_accession") col=i} NR>1 {print $col}' "$srr_list_file" > sralist.txt
...
# 校验 MD5
echo "开始 MD5 校验..."
awk -F'\t' 'NR>1 {print $2, $3}' "$srr_list_file" | while read -r run_accession sra_md5; do
    sra_file="${run_accession}/${run_accession}.sra"
    if [ -f "$sra_file" ]; then
...
# 文件重命名
echo "开始重命名文件..."
awk -F'\t' 'NR>1 {print $2, $1}' "$srr_list_file" | while read -r run_accession sample_title; do
    sra_file="${run_accession}/${run_accession}.sra"
    if [ -f "$sra_file" ]; then
        mv "$sra_file" "${run_accession}/${sample_title}.sra"
